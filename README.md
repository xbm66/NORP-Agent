...
你的文件不会直接被传输到云端处理，只会被大模型使用工具调用读取。若使用本地部署大模型，则可以实现真正的**数据不上云**
...
下载后双击运行即可。首次启动时输入你的 [API Key]及API供应商（若本地部署则不需要提供密钥），选择工作区目录，即刻开始编程。
...
| 🔵 **状态指示灯** | 标签页标题旁显示运行状态：绿色常量=执行中、蓝色闪烁=等待用户输入、橙色闪烁=等待确认 |
...
NORP Agent 的工具执行是一套**深度异步化的并发执行引擎**：
...
默认开启项，可手动关闭。当 AI 尝试执行**删除文件、覆盖文件**等破坏性操作时，系统会弹出确认对话框，**必须由用户手动批准**才能继续——AI 无法绕过。

---

## 🛠️ 内置工具集

Agent 共拥有 **16 个内置工具** 和 **32 个插件扩展工具**，覆盖编程全流程：
...
├── ═══ 插件系统 ═══
├── plugin_system/
│   ├── manager.py           #   插件加载/热重载/16 个 Hook 调度
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `model` | `deepseek-v4-pro` | 模型选择（自动从BaseURL拉取） |
