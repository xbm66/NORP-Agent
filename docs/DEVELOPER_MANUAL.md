...
> **版本**：0.5.0 ｜ **许可**：Copyright (c) 2026 xingluosama121, MIT Licensed
...
3. `"pkg.mod:attr;键=值;键=值"` —— 分号后附加配置，注入工厂的 `config` 参数
   （例如 `"norpagent.frontends.web:WebFrontend;port=9000"`）。
...
（`StdLoopRuntime.run_async` 的跨线程等待因此改用轮询而非回调。）
...
| 前端 | 地址 | 说明 |
|---|---|---|
| Web（默认） | `norpagent.frontends.web:WebFrontend` | HTTP + SSE，无第三方依赖；页面 = front.html（多标签会话/流式渲染/设置/插件面板）；控制台打印 `listening on http://127.0.0.1:8787/`；可配 `;port=9000` 或 `np(port=9000, language="zh_CN")` |
...
| 级别 | 输入防护 | 提示词加固 | 人工审批 | 网络策略 | 审计 |
|---|---|---|---|---|---|
| basic | 有 | - | - | - | - |
| standard | 有 | 有 | 有 | 有 | - |
| high | 有 | 有 | 有（严格） | 有 | 有 |

SafetyKit 提供 6 个独立检查 API，可单独使用。
插件加载继承当前安全级别；未装 cryptography 时签名校验
返回「不受信任」，不放行。
...
| L3 输入 | before_input / after_input | before | user_input, params |
...
| L7 模型 | before_model_call / after_model_call / on_content / on_usage_update | 两者 | messages, output, params |
...
*NorpAgent 开发手册 · v0.5.0 · Copyright (c) 2026 xingluosama121, MIT Licensed*
