# ============================================================================
# nasync_io - 自研异步 IO 库（事件循环核心完全自研，不调用标准 asyncio）
#
# 设计目标：
#   1. 事件循环调度核心（Task 协程 trampoline / 定时器堆 / 线程安全唤醒）全部自研，
#      以获得对异步循环逻辑的完全掌控（自由度）
#   2. 底层仅依赖 Python 标准库的非 asyncio 模块：
#      threading / queue / heapq / selectors / socket /
#      concurrent.futures / subprocess / os / time
#   3. 提供与现有业务代码中 asyncio 用法一一对应的同名 API，
#      业务文件只需把 import asyncio 改为 import nasync_io 即可完成移植
#
# 线程模型：
#   - 一个 NEventLoop 绑定一个线程（与现有架构一致：每个 Agent 会话一个循环线程）
